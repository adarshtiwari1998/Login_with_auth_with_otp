...
import { registerUser, sendRegOTP, verifyRegOTP } from '../helper/helper';
...
export default function Register() {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      username: 'example123',
      password: 'admin@123',
      otp: '', // Add this line for the OTP field
    },
    ...
    onSubmit: async (values) => {
      ...
      if (showOTPField) {
        try {
          let { status } = await verifyRegOTP({ username: values.username, code: values.otp });
          if (status === 201) {
        ...
        let otpSent = await sendRegOTP(values.username);
        if (otpSent) {
          ...
          <form className='py-1' onSubmit={formik.handleSubmit}>
              ...
              <div className="textbox flex flex-col items-center gap-6">
                  ...
                  {showOTPField && (
                    ...
                    style={{ display: showOTPField ? 'block' : 'none' }} // Hide or show based on state
                  />
              )}
